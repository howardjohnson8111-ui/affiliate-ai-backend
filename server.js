...
const { detectSuspicious } = require('./securityLockdown');
const { requestPayout } = require('./payoutService');
const { generateSecret, generateQRCode, verifyToken, enable2FA, is2FAEnabled } = require('./twoFactorService');
const { recordAction, getPattern, simulateAutopilot } = require('./patternLearner');
...
if (SUPABASE_URL && SUPABASE_ANON_KEY) {
  ...
  logger.log('⚠️  [Supabase]: Not configured. Set SUPABASE_URL and SUPABASE_ANON_KEY environment variables.');
}
...
async function protect(req, res, next) {
  try {
    ...
    if (!token) {
      await log('AUTH_FAILED', null, { ip: req.ip, userAgent: req.get('User-Agent') });
    ...
    if (error || !user) {
      await log('AUTH_FAILED', null, { ip: req.ip, userAgent: req.get('User-Agent') });
      logger.error('❌ [Auth Error]:', error ? error.message : 'User not found with token');
      return res.status(401).json({ error: '❌ Not authorized - invalid token' });
    }

    await log('LOGIN_SUCCESS', user.id, { ip: req.ip, userAgent: req.get('User-Agent') });
    recordAction(user.id, 'login', { ip: req.ip, userAgent: req.get('User-Agent') });
    pingDeadMans(user.id);
    req.user = user; // Attach the user object to the request
    next(); // Continue to the next middleware/route handler
  } catch (err) {
    await log('AUTH_ERROR', null, { ip: req.ip, userAgent: req.get('User-Agent') });
...
app.post('/api/campaigns', protect, validateCampaign, (req, res) => {
  try {
    ...
    // Record pattern
    recordAction(req.user.id, 'create_campaign', { name, platform });
...
app.post('/api/external-stocks/subscriptions/:id/notify', protect, async (req, res) => {
  const sub = stockSubscriptions[req.params.id];
  if (!sub) return res.status(404).json({ error: 'Subscription not found' });

  logger.log(`[Notifications]: Trigger test notification for ${sub.id} (symbol: ${sub.symbol})`);
  if (req.user?.phone) {
    await sendSMS(req.user.phone, `Affiliate AI: Alert for ${sub.symbol} triggered.`);
  }
...
app.get('/api/sessions', protect, async (req, res) => {
  const userId = req.user.id || 'demo-user';
  // In production: fetch from database; demo returns static list
  res.status(200).json([
    { id: 'sess-1', device: 'Chrome Windows', lastSeen: new Date().toISOString(), ip: req.ip },
    { id: 'sess-2', device: 'Safari iPhone', lastSeen: new Date(Date.now() - 86400000).toISOString(), ip: '10.0.0.2' }
  ]);
});

app.post('/api/sessions/revoke', protect, async (req, res) => {
  const { sessionId } = req.body;
  logger.log(`[Sessions]: User ${req.user.id} revoked session ${sessionId}`);
  res.status(200).json({ message: 'Session revoked' });
});

// Payout request
app.post('/api/payouts', protect, async (req, res) => {
  const { amount, method, recipient } = req.body;
  if (!amount || !method || !recipient) {
    return res.status(400).json({ error: 'Amount, method, and recipient required' });
  }
  try {
    recordAction(req.user.id, 'payout', { amount, method, recipient });
    const result = await requestPayout(req.user.id, amount, method, recipient);
...
app.get('/api/payouts/methods', (req, res) => {
  res.status(200).json({ methods: ['paypal', 'cash_app', 'chime', 'bank_transfer', 'crypto'] });
});

// 2FA setup
app.post('/api/2fa/setup', protect, async (req, res) => {
...
app.post('/api/2fa/enable', protect, (req, res) => {
  const { token } = req.body;
  if (!token) return res.status(400).json({ error: 'Token required' });
  const valid = verifyToken(req.user.id, token);
  if (!valid) return res.status(400).json({ error: 'Invalid token' });
  enable2FA(req.user.id);
  res.status(200).json({ message: '2FA enabled' });
});

// Verify 2FA during login (optional: step-up verification)
app.post('/api/2fa/verify', protect, (req, res) => {
  const { token } = req.body;
  if (!token) return res.status(400).json({ error: 'Token required' });
  const valid = verifyToken(req.user.id, token);
  if (!valid) return res.status(400).json({ error: 'Invalid token' });
...
// Autopilot: simulate actions based on learned patterns
app.get('/api/autopilot/simulate', protect, (req, res) => {
  const decisions = simulateAutopilot(req.user.id);
  if (!decisions) {
    return res.status(200).json({ message: 'Not enough data to simulate autopilot' });
  }
  res.status(200).json(decisions);
});

// Get learned patterns (read-only)
app.get('/api/patterns', protect, (req, res) => {
  const pattern = getPattern(req.user.id);
  if (!pattern) {
    return res.status(200).json({ message: 'No patterns learned yet' });
  }
  // Strip sensitive timestamps for UI
  const safe = {
    campaignFrequency: pattern.campaignFrequency,
    preferredPlatforms: pattern.preferredPlatforms,
    payoutMethods: pattern.payoutMethods,
    recentActionsCount: pattern.lastActions.length
  };
  res.status(200).json(safe);
});

// Dead Man's Switch: configure
app.post('/api/dead-mans/configure', protect, (req, res) => {
  const { enabled, inactiveDays } = req.body;
  configureDeadMans(req.user.id, { enabled, inactiveDays });
  res.status(200).json({ message: 'Dead Man’s Switch configured' });
});

// Dead Man's Switch: check status
app.get('/api/dead-mans/check', protect, async (req, res) => {
  const decisions = await checkDeadMans(req.user.id);
  if (decisions) {
    res.status(200).json({ triggered: true, decisions });
  } else {
    res.status(200).json({ triggered: false, message: 'Switch not triggered' });
  }
});

// Rules Engine: add rule
app.post('/api/rules', protect, (req, res) => {
  const { type, trigger, payload } = req.body;
  if (!type || !trigger || !payload) {
    return res.status(400).json({ error: 'Type, trigger, and payload required' });
  }
  const rule = addRule(req.user.id, { type, trigger, payload });
  res.status(200).json({ message: 'Rule added', rule });
});

// Rules Engine: remove rule
app.delete('/api/rules/:id', protect, (req, res) => {
  removeRule(req.user.id, req.params.id);
  res.status(200).json({ message: 'Rule removed' });
});

// Rules Engine: list rules
app.get('/api/rules', protect, (req, res) => {
  const rules = getRules(req.user.id);
  res.status(200).json(rules);
});

// Rules Engine: evaluate manually
app.post('/api/rules/evaluate', protect, async (req, res) => {
  const results = await evaluateRules(req.user.id);
  res.status(200).json({ message: 'Rules evaluated', results });
});
...
app.listen(PORT, () => {
  ...
  logger.log('\nAvailable endpoints:');
  logger.log('  POST   /api/campaigns       - Create a campaign');
  logger.log('  GET    /api/campaigns       - Get all campaigns');
  logger.log('  GET    /api/campaigns/:id   - Get a campaign by ID');
  logger.log('  PUT    /api/campaigns/:id   - Update a campaign');
  logger.log('  DELETE /api/campaigns/:id   - Delete a campaign');
  logger.log('  POST   /api/transactions    - Create a transaction');
  logger.log('  GET    /api/transactions    - Get all transactions');
  logger.log('  POST   /api/payments        - Create a payment');
  logger.log('  GET    /api/payments        - Get all payments');
  logger.log('  GET    /api/payments/:id    - Get payment details');
  logger.log('  PUT    /api/payments/:id    - Update payment status');
  logger.log('  POST   /api/payments/:id/verify - Verify PayPal payment');
  logger.log('  DELETE /api/payments/:id    - Cancel payment');
  logger.log('  GET    /api/paypal-config   - Get PayPal config');
