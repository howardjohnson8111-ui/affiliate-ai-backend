...
PERSONAS = {
    ...
    "marketing_manager": {
        ...
        "description": "Executes marketing commands, launches campaigns, and generates performance reports.",
        "tools": ["execute_marketing"]
    }
}
...
PERSONA_KEYWORDS = {
    ...
    "marketing_manager": ["marketing", "launch", "campaign", "report", "stats", "execute", "command", "ads", "promote"]
}
...
LANGUAGE_TOOLS = [
    ...
    types.FunctionDeclaration(
        name="execute_marketing",
        description="Execute a marketing command (launch campaign, generate report, etc.).",
        parameters=types.Schema(
            type=types.Type.OBJECT,
            properties={
                "command": types.Schema(type=types.Type.STRING, description="Marketing command to execute, e.g., 'launch campaign', 'generate report', 'show stats' (required)")
            },
            required=["command"]
        )
    )
...
def get_tools_for_persona(persona_key: str):
    ...
    available_tools = {
        ...
        "execute_marketing": LANGUAGE_TOOLS[3],
    }
...
def process_function_call(tool_name, tool_input):
    ...
    try:
        ...
        # Marketing command tool
        elif tool_name == "execute_marketing":
            command = tool_input.get("command", "").lower()
            print(f"[Marketing]: Executing command: {command}")
            if "launch" in command or "start" in command:
                return {
                    "status": "success",
                    "message": "Marketing campaign launched across all channels",
                    "actions": ["Email blast", "Social posts", "PPC ads"],
                    "next_step": "Monitor dashboard for clicks and conversions"
                }
            elif "report" in command or "stats" in command:
                return {
                    "status": "success",
                    "message": "Marketing report generated",
                    "metrics": {"clicks": 1240, "conversions": 34, "revenue": "$850"},
                    "next_step": "Optimize underperforming channels"
                }
            else:
                return {
                    "status": "success",
                    "message": f"Marketing command '{command}' executed",
                    "next_step": "Review results in Dashboard"
                }
